...
const _sfc_main = {
  name: "u-upload",
  mixins: [common_vendor.mpMixin, common_vendor.mixin, common_vendor.mixinUpload, common_vendor.props$10],
  data() {
    return {
      lists: [],
      isInCount: true,
  ...
  methods: {
    ...
    loadedVideoMetadata(e) {
      ...
      common_vendor.index.createSelectorQuery().select("#myVideo").context((res) => {
        common_vendor.index.__f__("log", "at node_modules/uview-plus/components/u-upload/u-upload.vue:291", "select video", res);
        const myVideo = res.context;
        common_vendor.index.createSelectorQuery().select("#myCanvas").fields({ node: true, size: true }).exec(([res2]) => {
          common_vendor.index.__f__("log", "at node_modules/uview-plus/components/u-upload/u-upload.vue:297", "select canvas", res2);
          ...
          const captureFirstFrame = () => {
            ...
            common_vendor.wx$1.canvasToTempFilePath({
              canvas: res2[0].node,
              success: (result) => {
                common_vendor.index.__f__("log", "at node_modules/uview-plus/components/u-upload/u-upload.vue:312", "First frame image path:", result.tempFilePath);
                this.fileList["currentItemIndex"].thumb = result.tempFilePath;
              },
              fail: (err) => {
                common_vendor.index.__f__("error", "at node_modules/uview-plus/components/u-upload/u-upload.vue:318", "Failed to export image:", err);
              }
            });
          };
          setInterval(() => {
